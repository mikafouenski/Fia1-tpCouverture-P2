...
public class Magasin {
	...
	// Recherche pourri seullement par prix ? Et le titre ? (pas demandé)
	public Boolean chercherDichoEtu(Article article) {
		int start = 0;
		int end = this.nbArticle - 1;
		while(start <= end) {
			int middle = (start + end) / 2;
			if (article.getPrix() < this.stock[middle].getPrix()) {
				end = middle - 1;
			} else if (article.getPrix() > this.stock[middle].getPrix()) {
				start = middle + 1;
			} else {
				// Prix equals, et le reste ?
				return true;
			}
		}
		return false;
